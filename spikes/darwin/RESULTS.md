...
**条件付き成立(技術統合は成立、精度は本基準音声セットでは不成立。実音声での再検証を推奨)。**
...
3. **不成立の主要因は基準音声側にある可能性が高い**: 基準音声は`say`コマンドによるTTS合成音声(`spikes/web`と共用の同一アセット)であり、人間の自然発話と異なる音響特性(抑揚・間・発音の均一さ)を持つ。特に数値(「128名」「128」)・固有名詞(会社名・人名)の読み上げでの誤認識が目立ち、これはTTS合成音声特有のパターンにSpeechAnalyzerが適応しにくいことを示唆する。人間の自然発話に対する精度は本スパイクでは未検証であり、別途確認が必要。
...
以上を踏まえ、tasks.mdの「M0 出口判定」に対しては、**Darwinを対象外化するのではなく、「技術統合は成立、精度基準は要再検証」という条件付きの成立として扱い、人間の自然発話による基準音声セットでの再検証をM1着手前の追加タスクとすることを提案する**。
