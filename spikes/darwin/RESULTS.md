...
確認できたのは、**macOS 26.5.1 と iOS 27.0 の2環境で ja-JP が `supportedLocales` に含まれる**という事実に限られる。`supportedLocales` はプラットフォーム・OSバージョンで件数・内容が異なることが本検証で実測されている以上、この2点から「iOS 26 以上で利用可能」と一般化することはできない。iOS 26 実機での確認は残課題である。
