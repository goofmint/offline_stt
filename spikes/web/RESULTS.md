...
> **追記(Issue #20 で確定)**: 上記の「非対応の方針とする」という結論は**採用されなかった**。ライブラリが勝手に速度を決めるのではなく、トレードオフを提示したうえで利用者に選ばせる方針が採られ、`TranscribeRequest.playbackRate`(既定 1.0、Web専用)として公開されている。本節は M0 時点の記録として残す。確定内容は design.md §8 未決事項7 を参照。
