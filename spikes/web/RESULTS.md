...
**このファイルは部分的な実機検証結果である。** Chrome 153 での `available()` / `install()` の検証は完了しており、実測値を記載している。`start(audioTrack)` の併用動作(Issue #5)と基準音声での精度確認(Issue #6)は実行環境の制約により未実施であり、該当箇所は `(未実施 — 環境要因。下記「実行環境の制約」参照)` と明示している。未実施項目の判定には、通常の対話的な Chrome セッションでの手動実行が必要である。
...
| `processLocally` 読み戻し値(設定直後) | `true`(設定単体では確認済み。audioTrack との併用は未実施 — 環境要因。下記「実行環境の制約」参照) |
