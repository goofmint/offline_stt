...
- [ ] 基準音声セット作成: ja-JP / en-US、各10秒・3分、wav と m4a(期待テキスト付き)
- [ ] 判定基準の定義(キーワード包含率のしきい値)
...
- [ ] iOS 26実機で SpeechTranscriber の supportedLocales に ja が含まれるか確認(設計未決事項3)
- [ ] AVAudioFile → SpeechAnalyzer のファイル入力スパイク(Swift単体、Flutter外)
- [ ] ファイル処理速度の実測(実時間比)
- [ ] macOS 26 でも同スパイクを確認
