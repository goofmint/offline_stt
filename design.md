...
- 正規化ルール: 一致判定の前に、期待キーワードと認識結果テキストの**両方**へ同一の正規化を同一手順で適用する。片側のみへの適用は禁止する
  ...
  - 許容表記の表現形式(`.json` の `keywords` のスキーマ): `keywords` は**キーワードグループの配列**である。1要素が1グループであり、要素は次の2形式のいずれかを取る
    - **文字列**: 表記が1つだけのキーワード(例: `"東京都渋谷区"`)
    - **文字列の配列**: 許容表記を列挙したキーワード(例: `["2024年11月3日", "2024/11/3", "2024-11-03"]`)
    - 判定は「グループ内のいずれか1表記が一致すれば、そのグループを一致とみなす」。**包含率の分母は表記数ではなくグループ数である**(表記を増やしても分母は増えない)
    - 文字列形式は配列形式の要素数1と等価であり、両形式は同一ファイル内で混在してよい
  - 許容表記として列挙してよいのは「**同じ内容を別の表記で書いたもの**」に限る(例: `three hundred and twenty thousand` / `320,000`、`forty five seconds` / `45 seconds`、`2024年11月3日` / `2024/11/3`)。**認識器がそう出力したというだけの誤認識を列挙してはならない**(例: `ISO 27001` に対する `ISO 2701`、`株式会社モーンギフト` に対する `モンギフト`、`Moongift Incorporated` に対する `Moon Gifting Corporated`、`128名` に対する `102十 8名`)。列挙は測定の不備を正すためのものであり、数値を良く見せるためのものではない
  - 各表記の選定理由は `test-assets/baseline-audio/README.md`「許容表記の一覧と選定理由」に記録する
