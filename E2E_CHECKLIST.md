...
`{clip}.json` の `keywords` は**キーワードグループの配列**であり、1要素は
文字列(表記が1つ)または文字列の配列(許容表記の列挙)を取る。グループ内の
いずれか1表記が一致すればそのグループを一致とみなし、**分母はグループ数**
である(表記を足しても分母は増えない)。列挙してよいのは「同じ内容の別表記」
だけであり、誤認識を足してはならない。詳細は
`test-assets/baseline-audio/README.md`「`keywords` のスキーマ」。

### しきい値(design.md §7)

| 条件 | 言語 | 判定 |
|---|---|---|
| クリーン基準音声 | ja-JP | **95%以上 = 合格** / **90〜94% = 条件付き合格(要確認)** / **90%未満 = 不成立** |
| クリーン基準音声 | en-US | **95%以上 = 合格** / 95%未満 = 不成立 |
| 実環境(ノイズあり、参考値) | 共通 | 上記から一律5ポイント程度緩和した値を参考とする |

**90〜94% は「合格」ではない。** ja-JP のみに置かれた中間帯であり、
条件付き合格(要確認)として扱う。

### 精度に関する既知の事実(重要)

**ja-JP はいずれのプラットフォームでもしきい値に達していない**(`jaJP_10s` は Darwin / Web / Android とも 66.7%)。**en-US は Android(Pixel 6)の `enUS_10s` が 100.0% でしきい値(95%以上)を満たしている**唯一の例である。
...
**`jaJP_10s` の 66.7% は前後で変わらない**(不一致は表記差ではなく誤認識で
あるため)。値が動いたのは3分クリップの一部であり、詳細は各 E2E_RESULTS.md。
