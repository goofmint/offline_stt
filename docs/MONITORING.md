...
**四半期ごとに、依存先の変更有無にかかわらず基準音声によるE2Eを再実行する**
(`tasks.md`「継続タスク(公開後)」の定め)。変更を検知した四半期はその範囲を
広げ、検知しなかった四半期も**最低限 `jaJP_10s` の包含率だけは測り直して
記録する**。オンデバイスの認識モデルはOS・ブラウザの自動更新に追従するため、
こちらが何も変えていなくても結果が動きうるからである。変更を検知した場合は
四半期を待たずにその時点で再実行する。

手順は既存のチェックリストをそのまま使い、本書では重複させない。
