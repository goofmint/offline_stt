...
手順書は [E2E_CHECKLIST_DARWIN.md](E2E_CHECKLIST_DARWIN.md)、包含率の算出規則は
[リポジトリルートの E2E_CHECKLIST.md](../../E2E_CHECKLIST.md) および
design.md §7。

**本書は Darwin(iOS/macOS)実装(本番実装。実行当時は `packages/offline_stt_darwin/`
パッケージだったが、Issue #91 の単一パッケージ統合後は `packages/offline_stt/darwin/`
〈Swift〉と `packages/offline_stt/lib/src/darwin/`〈Dart〉に配置されている)を
iOS 実機と macOS 実機で動かした初めての記録である。** `spikes/darwin/RESULTS.md` は M0 スパイク
...
**是正前のキーワードセットで採点した値**は M0 スパイクで macOS 上に記録
された値と一致した。すなわち **本番実装への移植による精度の劣化は無く、
同時に M0 時点の未達も解消していない。**
...
| 被検体 | 実行当時の `packages/offline_stt_darwin/`(Dart + Swift)+ `packages/offline_stt_platform_interface/`。単一パッケージ統合後は `packages/offline_stt/darwin/` + `packages/offline_stt/lib/src/{darwin,}/` |
...
方式を採った。理由は Android(`docs/e2e/E2E_RESULTS_ANDROID.md`)
と同じである。
...
(`docs/e2e/E2E_RESULTS_ANDROID.md`)。Darwin 固有の問題では
...
iOS では12秒間 0% のまま止まって見える。`packages/offline_stt/README.md`
...
- **是正前のキーワードセットで採点した値は、M0 スパイク(macOS、
  `.progressiveTranscription`)の値と一致した。** 本番実装への移植で精度は
  変わっていない。なお **M0 の記録値は是正前キーワードセットでの採点値で
  あり、是正後の 3分クリップの値(`enUS_3m` 80.0%、`jaJP_3m` 42.9% /
  32.1%)とは一致しない。** 採点規則が変わったためであって、認識結果が
  変わったわけではない。
...
**Darwin実装のコードは1行も変更していない**(実行当時の `packages/offline_stt_darwin/`。
現在は `packages/offline_stt/darwin/` + `packages/offline_stt/lib/src/darwin/`)。
バグが見つからなかったためである。
