...
手順書は [E2E_CHECKLIST_ANDROID.md](E2E_CHECKLIST_ANDROID.md)、包含率の算出規則は
[リポジトリルートの E2E_CHECKLIST.md](../../E2E_CHECKLIST.md) および
design.md §7。

**本書は Android実装(本番実装。実行当時は `packages/offline_stt_android/`
パッケージだったが、Issue #91 の単一パッケージ統合後は `packages/offline_stt/android/`
〈Kotlin〉と `packages/offline_stt/lib/src/android/`〈Dart〉に配置されている)を
実機で動かした初めての記録である。** `spikes/android/RESULTS.md` は M0 スパイク
...
| 被検体 | 実行当時の `packages/offline_stt_android/`(Dart + Kotlin)+ `packages/offline_stt_platform_interface/`。単一パッケージ統合後は `packages/offline_stt/android/` + `packages/offline_stt/lib/src/{android,}/` |
...
3. `packages/offline_stt/android/src/androidTest/` に instrumented
...
となり、spikes/android/RESULTS.md の値を再現する。**この性質は後述の
キーワードセット是正後も変わらない**(不一致2件はいずれも表記差ではなく
誤認識であるため)。

> **2026-09-21 追記: キーワードセット是正と再採点。** 本書が下記
> 「残る問題」で指摘していた「`enUS_3m.json` が design.md §7 の
> 『許容表記を列挙する』規定に従っていない」不備を**是正した**。
> `test-assets/baseline-audio/*.json` の `keywords` を
> 「文字列 または 許容表記の配列」スキーマに改め、`320,000` /
> `45 seconds` / `96.4%` のような**同じ内容の別表記**を列挙した
> (分母であるキーワードグループ数は変えていない)。
> 本書に**逐語で記録されている確定テキストだけ**を採点し直した結果を
> 各表に「是正後」として併記する。**端末での再測定は行っておらず、
> 認識結果は1文字も変わっていない。** 経緯と各表記の選定理由は
> design.md §7 および `test-assets/baseline-audio/README.md` を参照。
...
| クリップ | 確定テキストでの包含率(是正前) | **是正後** | 判定 | partial全体での包含率(参考、是正前) |
|---|---|---|---|---|
| jaJP_3m.wav | 5/28 = 17.9% | **5/28 = 17.9%**(変化なし) | 不成立 | 23/28 = 82.1% |
| jaJP_3m.m4a | 5/28 = 17.9% | 再採点不可(確定テキスト未記録) | 不成立 | 22/28 = 78.6% |
| enUS_3m.wav | 1/25 = 4.0% | **3/25 = 12.0%** | 不成立 | 11/25 = 44.0% |

是正後に HIT へ変わったのは `enUS_3m.wav` の `November 10th, 2024`
(認識結果 `November 10 2024` ←許容表記 `November 10, 2024` で一致)と
`forty two engineers`(認識結果 `42 engineers`)の2件である。
`jaJP_3m.m4a` と partial 全体の値は、対象テキストが本書に逐語で
記録されていないため再採点できない。
...
**キーワードセット是正の前後で値は変わらない**(下表は是正前・是正後で
同値)。不一致の2件(`東京都渋谷区` → `東京都渋谷で`、
`株式会社モーンギフト` → `株式会社モンギフト`)はいずれも表記差ではなく
誤認識であり、許容表記として列挙してはならないものだからである。
...
| 7. オフライン確認 | **未実施** |
| 8. 包含率(記録項目) | ja-JP 10秒 66.7%(M0 と同値)、en-US 10秒 100%(**しきい値を満たす**)、3分クリップは B-3 のため 4.0〜17.9%(キーワードセット是正後の再採点では 10秒は変化なし、enUS_3m.wav が 4.0% → 12.0%) |
...
| クリップ | 修正前 | 修正後 | 判定 | 確定テキスト長 / 期待 | キーワードセット是正後の再採点 |
|---|---|---|---|---|---|
| jaJP_10s.wav | 66.7% | **66.7%** | 不成立 | 51 / 57 | **66.7%**(変化なし) |
| jaJP_10s.m4a | 66.7% | **66.7%** | 不成立 | 51 / 57 | **66.7%**(変化なし) |
| enUS_10s.wav | 100.0% | **100.0%** | **合格** | 154 / 159 | **100.0%**(変化なし) |
| enUS_10s.m4a | 100.0% | **100.0%** | **合格** | 154 / 159 | **100.0%**(変化なし) |
| jaJP_3m.wav | 17.9% | **82.1%** | 不成立 | 1,054 / 1,151 | 再採点不可(確定テキスト未記録) |
| jaJP_3m.m4a | 17.9%(注) | **78.6%** | 不成立 | 1,058 / 1,151 | 再採点不可(確定テキスト未記録) |
| enUS_3m.wav | 4.0% | **32.0%** | 不成立 | 3,779 / 2,888 | 再採点不可(確定テキスト未記録) |
| enUS_3m.m4a | 4.0%(注) | **32.0%** | 不成立 | 3,761 / 2,888 | 再採点不可(確定テキスト未記録) |

(注) 修正前は B-1 のため wav/m4a のどちらかしか成功しない実行があった。

最右列は 2026-09-21 のキーワードセット是正(design.md §7 の「許容表記を
列挙する」規定への追従)後に、**同じ確定テキストを採点し直した**値である。
10秒クリップは本書に確定テキストが逐語で残っているため再採点できたが、
**3分クリップは本実行の確定テキストが逐語で記録されていないため再採点
できない**(本書に逐語で残っているのは初回実行の、B-3 で末尾セグメントだけに
なったテキストである。そちらは「手順8」に再採点値を併記した)。
...
> **2026-09-21 追記: この不備は是正済みである。** `keywords` を
> 「文字列 または 許容表記の配列」スキーマに改め、上記3件を含む表記差を
> 許容表記として列挙した(`320,000` / `96.4%` / `45 seconds` 等)。
> 分母(キーワードグループ数)は変えていない。ただし**本節が対象としている
> 実行の確定テキストは本書に逐語で記録されていないため、32.0% という値を
> 採点し直すことはできない。** 逐語で残っている Darwin の同一クリップでは
> 44.0% → **80.0%** になっており、同種の改善が Android でも起きる見込みは
> あるが、**測っていない値を推定で書くことはしない。**
...
| クリップ | 修正前 | 最終 | 判定 | 確定テキスト長 / 期待 | キーワードセット是正後の再採点 |
|---|---|---|---|---|---|
| jaJP_10s.wav | 66.7% | **66.7%** | 不成立 | 51 / 57 | **66.7%**(変化なし) |
| jaJP_10s.m4a | 66.7% | **66.7%** | 不成立 | 51 / 57 | **66.7%**(変化なし) |
| enUS_10s.wav | 100.0% | **100.0%** | **合格** | 154 / 159 | **100.0%**(変化なし) |
| enUS_10s.m4a | 100.0% | **100.0%** | **合格** | 154 / 159 | **100.0%**(変化なし) |
| jaJP_3m.wav | 17.9% | **82.1%** | 不成立 | 1,059 / 1,151 | 再採点不可(確定テキスト未記録) |
| jaJP_3m.m4a | 17.9% | **78.6%** | 不成立 | 1,059 / 1,151 | 再採点不可(確定テキスト未記録) |
| enUS_3m.wav | 4.0% | **40.0%** | 不成立 | 4,305 / 2,888 | 再採点不可(確定テキスト未記録) |
| enUS_3m.m4a | 4.0% | **40.0%** | 不成立 | 4,311 / 2,888 | 再採点不可(確定テキスト未記録) |

最右列は 2026-09-21 のキーワードセット是正後に同じ確定テキストを採点し
直した値である。**認識のやり直しはしていない。** 3分クリップは本実行の
確定テキストが本書に逐語で記録されていないため再採点できない。
...
いるが `enUS_3m.json` が従っていない。ライブラリの不具合ではない。

> **2026-09-21 追記**: この不備は是正した(`keywords` を「文字列 または
> 許容表記の配列」スキーマに変更し、`320,000` などの表記差を列挙した)。
> **ただし 40.0% は再採点できない。** 本実行の確定テキストが本書に逐語で
> 記録されていないためである。**認識そのものは変わっていない。**
