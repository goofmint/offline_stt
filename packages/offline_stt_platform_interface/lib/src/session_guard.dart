...
///
/// ## この共有層に置く理由(Issue #23)
/// 各ネイティブ実装パッケージ(offline_stt_android / offline_stt_darwin /
/// offline_stt_windows / offline_stt_web)が個別にこの排他ロジックを実装
/// すると、実装が重複するうえ実装漏れも起きやすい。そのため共有層である
/// 本パッケージ(offline_stt_platform_interface)にガードを置き、各ネイティブ
/// 実装は [TranscribeSessionGuard] を `with` したうえで、
/// `transcribeFile()` の実装内で [guardSession] にネイティブセッションを
/// 開始するコールバックを渡す形で利用する想定である。
///
/// ## lib/src/ に置き、公開バレル(offline_stt_platform_interface.dart)
/// からはエクスポートしない理由
...
/// 一方でネイティブ実装パッケージ(本リポジトリのsiblingパッケージ)からは
/// `package:offline_stt_platform_interface/src/session_guard.dart` を直接
/// importして利用することを想定している。federated pluginで
/// プラットフォーム実装間だけに共有したい実装詳細を `src/` 経由で共有する
/// のは一般的なパターンである。
///
