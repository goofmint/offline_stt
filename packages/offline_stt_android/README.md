...
長時間の音声ではその長さ分だけ待つことになる。バッチ認識である Darwin / Windows とはこの点が根本的に異なるので、UI の進行表示はこの前提で設計すること。
...
Pixel 6(Android 17 / API 37)実機での実測は、基準音声 `jaJP_10s` で **66.7%(4/6)** であり、design.md §7 のしきい値(ja-JP 90%以上)に**達していない**。`株式会社モーンギフト` を「モンギフト」と誤認識している。
