...
/// テストする。テスト方針の詳細は`src/android/offline_stt_android.dart`
/// 冒頭コメントを参照)。`DownloadProgress`はフィールドをそのまま写す
/// だけで分岐ロジックを持たないため、専用の写像関数には切り出さず
/// 呼び出し箇所で直接組み立てている(Darwin実装と同一の構成)。
...
/// `hostApi.downloadModel()`を呼び始める(Darwin/Web実装の
