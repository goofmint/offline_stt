...
void main() {
  ...
  testWidgets('手順1-0: supportedLocales', (tester) async {
    ...
    // ロケールの集合」を返す(Darwin実装は `SpeechTranscriber.supportedLocales` をそのまま写す)。
  ...
  testWidgets('手順5: セッション排他', (tester) async {
    ...
    final firstSegment = Completer<void>();
    Object? firstError;
    final first = const OfflineTranscriber()
        .transcribeFile(
          TranscribeRequest(path: '$assetDir/jaJP_3m.wav', locale: 'ja-JP'),
        )
        .listen(
  ...
  testWidgets('追加: ダウンロード中の文字起こしキャンセルが巻き添えにしないこと', (tester) async {
    ...
    final download = const OfflineTranscriber()
    ...
    // false である。
    final early = const OfflineTranscriber()
        .transcribeFile(
          TranscribeRequest(path: '$assetDir/jaJP_3m.wav', locale: 'ja-JP'),
        )
