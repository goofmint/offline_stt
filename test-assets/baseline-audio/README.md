...
2. **評価用キーワードリスト**(`{id}.json` の `keywords` 配列): 意味上重要な名詞・固有名詞・数値・専門用語から選定。10秒版は5件以上、3分版は15件以上を目安とする。表記揺れが起こりうるキーワード(日付・数値等)は許容表記を列挙してよく、いずれか1つに一致すれば当該キーワードを一致とみなす(design.md §7 参照)。
